...
proto.type = {
	...
	circular: function () {
		...
		self.renderValue = function (value) {
			...
			self.move(
				Math.cos(angle) * hScope * 0.5 + hScope * 0.5,
				Math.sin(angle) * vScope * 0.5 + vScope * 0.5
			);
		};
		...
		self.calcValue = function (e) {
			...
			var x = xy[0] - hScope * 0.5 + self.draggable.pin[0];
			var y = xy[1] - vScope * 0.5 + self.draggable.pin[1];
	...
	round: function () {
		...
		self.calcValue = function (e) {
			...
			var x = xy[0] + self.draggable.pin[0] - hScope * 0.5;
			var y = xy[1] + self.draggable.pin[1] - vScope * 0.5;
