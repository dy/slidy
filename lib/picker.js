...
function Picker (el, options) {
	...
	//generate self id
	self.id = getUid();
	if (!self.element.id) self.element.id = 'slidy-picker-' + self.id;
	...
	//FIXME: detect pageStep as step * 5 or less, native detector is obscure
	//FIXME: set initial value, if undefined, as range/2
...
Object.defineProperties(proto, {
	value: {
		...
		get: function () {
			...
			if (this._value === undefined) this.value = 50;
...
function handle2dkeys (keys, value, step, min, max) {
	...
	//pageup
	if (keys[33]) {
		if (isFn(step)) step = step(value[0] + MIN_STEP);
		value[0] += pageStep;
	}
...
function handle1dkeys (keys, value, step, min, max) {
	...
	//up and right - increase by one
	if (keys[38] || keys[39]) {
		if (isFn(step)) step = step(value + MIN_STEP);
		value += step;
	}
